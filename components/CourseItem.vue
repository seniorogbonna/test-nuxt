<template>
  <div @click="" class="d-flex w-60 accordion-item-section pointer">
    ...
    <div class="flex-full item-title">{{ courseItem.title }}</div>
    <div class="d-flex">
      <div class="text-sections">{{ courseItem.lectures }} lectures</div>
      <div class="pl-1">{{ courseItem.time }}min</div>
    </div>
  </div>
  <div v-if="courseItem.expanded">
    <div class="d-flex" v-for="(course, i) in courseItem.lectureDetail">
